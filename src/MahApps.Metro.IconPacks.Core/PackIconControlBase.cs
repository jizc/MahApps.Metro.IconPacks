...
namespace MahApps.Metro.IconPacks
{
    ...
    public abstract class PackIconControlBase : PackIconBase
    {
        ...
        {
            if (dependencyObject is PackIconControlBase packIcon
                && (!packIcon.IsVisible || packIcon.Opacity <= 0 || packIcon.SpinDuration <= 0.0))
            {
                return false;
            }
        ...
        private static void SpinPropertyChangedCallback(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            if (e.OldValue != e.NewValue
                && e.NewValue is bool newBool
                && dependencyObject is PackIconControlBase packIcon)
            {
        ...
        private static void SpinDurationPropertyChangedCallback(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            if (e.OldValue != e.NewValue
                ...
                && dependencyObject is PackIconControlBase { Spin: true } packIcon)
            {
        ...
        {
            if (e.OldValue != e.NewValue && dependencyObject is PackIconControlBase { Spin: true } packIcon)
            {
                packIcon.StopSpinAnimation();
                packIcon.BeginSpinAnimation();
            }
        ...
        private void BeginSpinAnimation()
        {
            ...
            Storyboard.SetTargetProperty(
                animation,
                new PropertyPath(
                    $"(0).(1)[{transformGroup.Children.Count - 1}].(2)",
